...
@section('page-subtitle', 'Isi formulir untuk menambahkan konselor baru')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-[#EDE1FA] p-6">
        <form action="{{ route('counselors.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="specialization" name="specialization" value="{{ old('specialization') }}" placeholder="contoh: Psikolog Klinis Dewasa"
                    class="w-full px-4 py-3 rounded-xl border border-[#D9C2F0] focus:outline-none focus:ring-2 focus:ring-purple-deep focus:border-transparent transition text-sm">
            </div>
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" id="sipp_number" name="sipp_number" value="{{ old('sipp_number') }}"
                        class="w-full px-4 py-3 rounded-xl border border-[#D9C2F0] focus:outline-none focus:ring-2 focus:ring-purple-deep focus:border-transparent transition text-sm">
                </div>
                <div>
            ...
            <div>
                ...
                <select id="status" name="status"
                    ...
                    <option value="active" class="text-emerald-600 font-semibold bg-white" style="color: #059669;" {{ old('status') === 'active' ? 'selected' : '' }}>Aktif</option>
                    <option value="inactive" class="text-red-500 font-semibold bg-white" style="color: #ef4444;" {{ old('status') === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                    class="w-full px-4 py-3 rounded-xl border border-[#D9C2F0] focus:outline-none focus:ring-2 focus:ring-purple-deep focus:border-transparent transition text-sm resize-none">{{ old('notes') }}</textarea>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="bg-purple-deep text-white px-6 py-3 rounded-xl text-sm font-semibold hover:opacity-90 transition">Simpan Konselor</button>
